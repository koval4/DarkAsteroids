...
void RoomGenerator::generate(Rectangle square) {
    ...
    // making walls
    auto make_wall = [&square, this, &wall_texture] (uint16_t i, uint16_t j) -> void {
        Tile::ptr tile = map->at({i, j});
        tile->set_passable(false);
        tile->set_texture(wall_texture);
    };

    for (uint16_t i = square.first.x; i <= square.last.x; i++) {
        make_wall(i, square.first.y);
        make_wall(i, square.last.y);
    }
    for (uint16_t i = square.first.y; i <= square.last.y; i++) {
        make_wall(square.first.x, i);
        make_wall(square.last.x, i);
    }

    // making floor
    for (uint16_t i = square.first.x + 1; i < square.last.x; i++) {
        for (uint16_t j = square.first.y + 1; j++ < square.last.y; j++) {
            Tile::ptr tile = map->at({i, j});
